...
typedef enum Part_Type { hp, rp } Part_Type;

void print_partvec(Part_Type pt) {

    string ext = pt == hp ? "hp" : "rp";
    ostringstream path;
    path << output_dir << file_name << "." << k_way << "." << ext;
    FILE * file = fopen(path.str().c_str(), "w");
    for (int i = 0; i < A.size(); i++) {
        fprintf(file, "%d ", partition_vector[i]);
    }
    fprintf(file, "\n");
...
void comm_volume(){
    ...
    for(int i = 0; i < A.size(); i++){
        ...
        for(auto e : A[i]){
            int j = e.first;
            conn.insert(partition_vector[j]);
        }
        comm += conn.size() - 1;
    }
    printf("comm: %d\n", comm);
...
int main(int argc, char** argv) {
    ...
    while ((c = getopt(argc, argv, "a:o:k:r")) != -1) {
        switch (c) {
            case 'a':
                path_to_A = string(optarg);
                file_name = path_to_A.substr(path_to_A.find_last_of('//')+1, path_to_A.size());
                break;
            case 'o':
    ...
    read_matrix_market(path_to_A, A, 0);
