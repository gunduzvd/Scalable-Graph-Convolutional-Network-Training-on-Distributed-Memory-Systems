...
def sample_adjacency_matrix(A, indices):
    filter = np.in1d(A.row, indices)
    row = A.row[filter]
    col = A.col[filter]
    data = A.data[filter]

    filter = np.in1d(col, indices)
    row = row[filter]
    col = col[filter]
    data = data[filter]

    return sparse.coo_matrix((data, (row, col)), shape=A.shape)
...
def communicate_fgm(H, backward = False):
    ...
    for (source, indices) in recv.items():
        ...
        h = torch.zeros(len(indices), H.shape[1], device=device)
        dist.recv(h, src=source)
        X[indices] = h

        stats["recv_volume"] += h.size(0)
        stats["recv_nmsg"] += 1
...
class SequentialGCN(nn.Module):
    def __init__(self, in_features, out_features):
        super(SequentialGCN, self).__init__()
        self.gcn1 = PGCN(in_features, out_features)
        self.gcn2 = PGCN(in_features, out_features)
        self.gcn3 = PGCN(in_features, out_features)
...
def run(rank, size, nlayers, nfeatures, path_A, path_partvec, backend, batch_size):
    global myrank, world_size, send_map, recv_map, device, X, \
        recv_buffers, cpu_device, cuda_device, recv_buffers, send_buffers, stats, nbatches, batch_indices, batch_i
    ...
    A = mmread(path_A)
    with open(path_partvec, 'rb') as file:
        partvec = pickle.load(file)

    nbatches = A.shape[0] // batch_size + 1
    nbatches = nbatches * 3

    batches = []
    for i in range(nbatches):
        indices =range(0, A.shape[0])
        batch_indices = np.array(random.sample(indices, batch_size))
        bA = sample_adjacency_matrix(A, batch_indices)
        batch_send_map, batch_recv_map = compute_communication_maps(bA, partvec, rank, size)
        bA = get_partitiont_of_adjacency_matrix(bA, partvec, myrank)
        batches.append([bA, batch_send_map, batch_recv_map])
    ...
    # model = nn.Sequential(
    #     *[PGCN(nfeatures,nfeatures) for _ in range(nlayers)]
    # )
    # model = PGCN(nfeatures, nfeatures)
    model = SequentialGCN(nfeatures, nfeatures)
    ...
    for epoch in range(1):
        ...
        for batch_i in range(nbatches):
            ...
            bA = batches[batch_i][0]
            send_map = batches[batch_i][1]
            recv_map = batches[batch_i][2]
    ...
    for epoch in range(4):
        ...
        if myrank == 0:
            print("Epoch {:05d} | Loss {:.4f}".format(epoch, loss_epoch))
    ...
    if myrank == 0:
        ...
        print(f"total_vol: {total_vol} total_nmsg: {total_nmsg}", flush=True)


def init_process(rank, size, fn, nlayers, nfeatures, path_A, path_partvec, backend, batch_size):
    ...
    fn(rank, size, nlayers, nfeatures, path_A, path_partvec, backend, batch_size)

    dist.destroy_process_group()


def main(argv):
    global path_A, path_partvec, backend, size, rank, batch_size
    size = int(os.environ["SLURM_NPROCS"])
    rank = int(os.environ["SLURM_PROCID"])
    os.environ["RANK"] = os.environ["SLURM_PROCID"]
    try:
        opts, args = getopt.getopt(argv, "a:p:b:s:l:f:n:", [])
    except:
    ...
    p = mp.Process(target=init_process, args=(rank, size, run, nlayers, nfeatures, path_A, path_partvec, backend, batch_size))
    p.start()
    p.join()
