...
def communicate_fgm(H, backward = False):
    global send_map, recv_map, X, recv_buffers, cpu_device, cuda_device, recv_buffers, send_buffers, stats
...
def run(rank, size, nlayers, nfeatures, path_A, path_partvec, backend):
    global myrank, world_size, send_map, recv_map, device, X, recv_buffers, cpu_device, cuda_device, recv_buffers, send_buffers, stats
    myrank = rank
    world_size = size
    if backend == "gloo":
    ...
    for epoch in range(4):
        ...
        if myrank == 0:
            print("Epoch {:05d} | Loss {:.4f}".format(epoch, loss), flush=True)
